...
mouseDownHandler pickerType state _ onChange =
    let
        stateValue =
            getStateValue state
    ...
    in
    onChange
        (InternalState <| updateTimeIndicator updatedStateValue)
        updatedDate
...
updateHourState stateValue _ moveData =
    let
        currentAngle =
...
updateMinuteState stateValue _ moveData =
    let
        currentAngle =
            DateTimePicker.Geometry.calculateAngle originPoint axisPoint (Point moveData.offsetX moveData.offsetY)
